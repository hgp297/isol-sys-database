...
ida_df = prepare_ida_util(design_dict, config_dict=my_config_dict)
row_num = 9
...
p_D = plc_cases['1.4D'] / ft / 1.4
w_D = w_cases['1.4D'] / ft / 1.4
...
m_grav_inner_D = w_D_total * L_bay / g 
m_grav_outer_D = w_D_total * L_bay / 2 /g 

# divide LC mass over rest of frame
m_lc = sum(p_lc) / g /n_bays
m_lc_d = sum(p_D) / g / n_bays
...
for nd in floor_nodes:
    ...
    if (bay == n_bays) or (bay == 0):
        ...
        m_nd_g = m_grav_outer_D + m_lc_d
    else:
        m_nd = m_grav_inner + m_lc
        m_nd_g = m_grav_inner_D + m_lc_d
        
    negligible = 1e-15
    ops.mass(nd, m_nd, m_nd, m_nd_g,
...
if bldg.isolator_system == 'TFP':
    ...
    L_bldg = bldg.L_bldg
    n_bays = bldg.num_bays
    # pOuter = w_D_total*(L_bay/2)*ft* ((L_bldg - L_bay)/L_bay) 
    # pInner = w_D_total*(L_bay)*ft* ((L_bldg - L_bay)/L_bay) 
...
diaph_elems = bldg.elem_tags['diaphragm']
for elem in diaph_elems:
    w_applied = np.sum(w_floor)
    # w_applied = np.sum(w_D)
    ops.eleLoad('-ele', elem, '-type', '-beamUniform', 
                -w_applied, 0.0)
...
# # TODO: here
# dist = bldg.D_m
# peaks = np.linspace(0.0, dist*2, 7)
...
plt.plot(isol_disp['column_0'], -isol_shear, color='peru', linewidth=1.5)
plt.plot(u_bearing, fs_bearing*isol_axial.max(), linestyle='--', color='red')
# plt.plot(isol_disp['column_0'], -isol_shear/isol_axial, color='peru', linewidth=1.5)
# plt.plot(u_bearing, fs_bearing, linestyle='--', color='red')
plt.xlabel('Displacement (in)', fontsize=axis_font)
plt.ylabel('Normalized lateral force', fontsize=axis_font)
plt.grid(True)

slope = max(abs(isol_shear))/max(abs(isol_disp['column_0']))
T_record = 2*pi*(bldg.W/2/(g*slope))**0.5
...
plt.plot(u_bearing, fs_bearing, linestyle='--', color='blue')
# plt.plot(-isol_disp['column_0'], isol_shear/(bldg.W/2), color='navy', linewidth=1.5)
# plt.plot(u_bearing, fs_bearing/(bldg.W/2), linestyle='--', color='blue')
plt.xlabel('Displacement (in)', fontsize=axis_font)
