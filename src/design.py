...
def iterate_on_Q_tfp(Q_guess, mu_1, S_1, T_m, zeta_target, rho_k):
    from numpy import interp
    g  = 386.4
    pi = 3.14159
    ...
    # from Q and D_m
    k_2 = (k_M*D_m - Q_guess)/D_m
    R_2 = 1/(2*k_2) + h_2
    
    # from rho_k
    u_a = Q_guess/(k_2*(rho_k-1))
    k_a = rho_k*k_2
    mu_2 = u_a*k_a
    R_1 = u_a/(2*(mu_2-mu_1)) + h_1
...
def design_TFP(param_df, mu_1=None):
    ...
    # design will achieve T_m, Q, rho_k as specified
    from scipy.optimize import minimize_scalar
    res = minimize_scalar(iterate_on_Q_tfp, args=(mu_1, S_1, T_m, zeta_m, rho_k),
                             bounds=(0.01, 0.20), method='bounded')
...
def design_TFP_legacy(param_df):
    ...
    if 'zeta_e' in param_df.index: 
        ...
        while mu_bad:
            ...
            tries += 1
        if mu_bad:
            raise ValueError('No realistic design possible.')
        else:
            k_a = k_a.real
            k_ratio_e = k_ratio_e.real
            mu_1 = mu_1.real
            mu_2 = mu_2.real
            u_a = u_a.real
...
def get_brace_demands(Fx, del_xe, q, h_story, L_bay, w_1, w_2):
    ...
    C_E = q/(2*cos(theta)) # 2 brace per frame
...
def interaction_equation(Zx, Pu, Pn, Mu):
    ...
    if Pu/Pn > 0.2:
        ...
        combined_forces_coef = Pu/(2*Pn) + (Mu/Mnx)
